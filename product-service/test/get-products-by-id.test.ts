...
import { mockProducts } from '../lambda-functions/data/mock-data';
...
describe('getProductById lambda function', (): void => {
  it('should return product by id and status 200', async (): Promise<void> => {
    ...
    const result = (await handler(event, context, (): void => {})) as APIGatewayProxyResult;
  ...
  it('should return status 400 if product ID is not provided', async (): Promise<void> => {
    ...
    expect(result.body).toBe(JSON.stringify({ message: 'Product id is not provided' }));
  });
});
